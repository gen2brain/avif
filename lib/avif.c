...
// libaom uses setjmp/longjmp for its internal error path; it is never taken for
// valid input, so stubbing them (no real wasm setjmp support) is sufficient.
int setjmp(int a) {
    return 0;
}
...
// dav1d links against pthread but runs entirely in the calling thread when
// n_threads == 1 (maxThreads == 1 above), so these are only here to satisfy the
// linker and are never invoked -- except pthread_once, which must run the
// one-time initializer (dav1d's DSP/table init).
